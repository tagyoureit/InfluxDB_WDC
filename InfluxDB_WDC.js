    ...
    function getDBs() {
        try {
            ...
            $('#getSchemaButton').click(function () {
                ...
                if ($('#querytype').find(":checked").data('val') === 'custom') {
                    parseCustomSql(db, $('#customSql').val())
                }
                else {
...
// Init function for connector, called during every phase
        ...
        try {
            ...
            var jqxhr = $.ajax({
                ...
                .done(function (resp) {
                    ...
                    var values, columns, tags, val, val_len, col, col_len, response_array, series, series_cnt, row,
                        total_rows;
    ...
    $(document).ready(function () {
        ...
        $('#authGroup').on('show.bs.collapse', function () {
            useAuth = true
            $('#reloadWithAuth').prop("hidden", "hidden");
            $('#reloadWithoutAuth').prop("hidden", "");

        })

        $('#authGroup').on('hide.bs.collapse', function () {
            useAuth = false
            $('#reloadWithAuth').prop("hidden", "");
            $('#reloadWithoutAuth').prop("hidden", "hidden");
        })


        // set defaults

        $('#aggregationGroup').collapse('hide');
        $('#customSqlGroup').collapse('hide');

        $('#querytype').on('change', function () {
            if ($(this).find(":checked").data('val') === 'aggregation') {
                $('#aggregationGroup').collapse('show');
                $('#customSqlGroup').collapse('hide');
                queryType = 'aggregation';
            }
            if ($(this).find(":checked").data('val') === 'all') {
                $('#aggregationGroup').collapse('hide');
                $('#customSqlGroup').collapse('hide');
                queryType = 'all';

            }
            else if ($(this).find(":checked").data('val') === 'custom') {
                $('#aggregationGroup').collapse('hide');
                $('#customSqlGroup').collapse('show');
                queryType = 'custom';
            ...
            if (tableau.connectionData.length > 0) {
                try {
                    ...
                    if (json.queryType === 'aggregation') {
                        ...
                        $('#customSqlGroup').collapse('show');
                        $('#customSql').val(json.customSql)
                        $('#aggregationGroup').collapse('hide');
                        $('#querytype_custom').click()
                    }
                    if (json.useAuth === true) {
                        useAuth = true;
                        $('#authGroup').collapse('show');
                        $('#reloadWithAuth').prop("hidden", "hidden");
                        $('#reloadWithoutAuth').prop("hidden", "");
                        $('#username').val(tableau.username);
                        $('#password').val('');
                    } else {
                        $('#authGroup').collapse('hide');
                        $('#reloadWithoutAuth').prop("hidden", "hidden");
                        $('#reloadWithAuth').prop("hidden", "");
                    }
                    $('#getSchemaButton').prop('disabled', false);
